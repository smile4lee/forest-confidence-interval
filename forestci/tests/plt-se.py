...
in_csv = r"/root/onedrive-ttu/research/brsoy.hpcc/train.{0}/tuned/out.corr/brsoy-yield-input.training.pred.csv".format(
    var)
test_in_csv = r"/root/onedrive-ttu/research/brsoy.hpcc/train.{0}/tuned/out.corr/brsoy-yield-input.test.pred.csv".format(
    var)
...
def do_fci(n_trees):
    ...
    pred_test_fci, se = fci.forestci.random_forest_standard_error(rfr, X_test)


    df_test['pred_test'] = pred_test
    df_test['pred_test_fci'] = pred_test_fci.round(0).astype(int)
    df_test['se'] = se.round(0).astype(int)
    ...
    out_csv = r"out.se/out.{0}.csv".format(n_trees)
